H5P.Column = (function (EventDispatcher) {
  ...
  function Column(params, id, data) {
     ...
     * @private
     */
    var completed = function () { console.log('completed');
      // Sum all scores
      var raw = 0;
      var max = 0;
     ...
     */
    var trackScoring = function (taskIndex) { console.log('trackScoringwa'+taskIndex);
      return function (event) { console.log(event);
        if (event.getScore() === null) { console.log('event.getScore()'+event.getScore());
     ...
     */
    var addRunnable = function (content, contentData) { console.log('addRunnable');
     ...
     * @private
     */
    var createHTML = function () { console.log('htmlwaing');
      // Create wrapper
	...
	H5P.JoubelUI.createButton({
        ...
        on: {
            click: function () {
              ...
              setTimeout(function () {
                createSummary(wrapper,tasksResultEvent);
              }, 0);
 ...
 var createSummary = function (wrapper,tasksResultEvent) {
      ...
      for (var i = 0; i < params.content.length; i++) {
        ...
        if(typeof event === 'undefined'){
            table_content += '<tr>';
          table_content += '<td>'+content.content.metadata.title+'  (Skipped) </td>';
        ...
        //console.log(content);
        table_content += '<tr>';
        table_content += '<td>'+content.content.metadata.title+'</td>';
        table_content += '<td>'+event.getScore()+'/'+event.getMaxScore()+'</td>';
        table_content += '</tr>';
      }
      table_content += '</tbody>';
      //console.log(table_content);
      var summary_html = '<div class="custom-summary-section"><div class="h5p-summary-table-pages"><table class="h5p-score-table"><thead><tr><th>Content</th><th>Score/Total</th></tr></thead>'+table_content+'</table></div></div>';
      
      H5P.jQuery(wrapper).append(summary_html);
